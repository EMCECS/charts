...
CHARTS := ecs-cluster objectscale-manager mongoose zookeeper-operator atlas-operator decks kahm srs-gateway dks-testapp fio-test sonobuoy dellemc-license service-pod objectscale-graphql helm-controller objectscale-vsphere iam pravega-operator bookkeeper-operator
...
HELM_MONITORING_ARGS = --set global.monitoring_tag=3.7.0.0-969.de6276b2
...
create-manager-app: create-temp-package
	...
	--set global.rsyslog_enabled=true \
